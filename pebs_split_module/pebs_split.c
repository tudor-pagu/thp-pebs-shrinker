...
struct task_struct *thp_split_thread = NULL;
...
static void check_huge_pages(void) {
    ...
    xa_for_each(&thp_xarray, thp_index, info) {
        ...
        if (used_subpages < 64) {
            if (!pfn_valid(thp_index)) {
                pr_err("check_huge_pages: invalid pfn number 0x%lx\n", thp_index);
                continue;
            }

            struct page *sparse_page = pfn_to_page(thp_index);

            if (!PageTransHuge(sparse_page)) {
                pr_err("check_huge_pages: a page is not huge 0x%lx\n", thp_index);
                continue;
            }

            pr_info("check_huge_pages: decided to split a page 0x%lx, density=%d\n", thp_index, used_subpages);
            split_huge_page(sparse_page);
...
static int __init pebs_split_init(void) {
    ...
    pr_info("PEBS sampler events created\n");

    thp_split_thread = kthread_run(thp_split_thread_func, NULL, "thp_split_thread");

    pr_info("PEBS module loaded\n");

    return 0;
}   

static void __exit pebs_split_exit(void) {
    pr_info("Huge pages info:\n");

    if (thp_split_thread) {
        kthread_stop(thp_split_thread);
    }
...
module_init(pebs_split_init);
module_exit(pebs_split_exit);
